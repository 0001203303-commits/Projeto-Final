...
@section('conteudo')
    <style>
        ...
        .form-editar-expanded form,
        .form-cadastro-container form {
            max-width: 100%;
            margin: 0;
            padding: 10px;
        ...
        .table-pacientes tbody tr:not(.form-editar-expanded):hover {
            background-color: #f8fafc;
        }
    ...
    <main class="main-content">
        <div class="header"
            ...
            <div>
                ...
                <p style="color: #64748b; margin: 0; font-size: 0.95rem;">Controle de fila e classificação de risco
                    (Manchester).</p>
            </div>
            <div style="display: flex; gap: 10px; align-items: center;">
                <div class="search-box">
                    <input type="text" id="inputBusca" placeholder="Buscar por nome..." onkeyup="filtrarTabela()"
                        style="padding: 10px 14px; border-radius: 8px; border: 1px solid #cbd5e1; font-size: 0.9rem; outline: none;">
                </div>
        ...
        @if(session('error'))
            <div
                style="background: #fef2f2; border: 1px solid #fee2e2; color: #991b1b; padding: 12px 16px; border-radius: 6px; margin-bottom: 16px; font-weight: 600;">
                {{ session('error') }}
            </div>
        @endif
        @if(session('success'))
            <div
                style="background: #f0fdf4; border: 1px solid #bbf7d0; color: #166534; padding: 12px 16px; border-radius: 6px; margin-bottom: 16px; font-weight: 600;">
                {{ session('success') }}
            </div>
        @endif
            ...
            <form action="{{ route('pacientes.salvar') }}" method="POST">
                ...
                <div class="form-grid-3">
                    <div>
                        ...
                        <input type="text" id="new-cpf" name="cpf" class="mascara-cpf" placeholder="000.000.000-00"
                            required>
                    </div>
                    <div>
                        <label for="new-protocolo">Protocolo:</label>
                        <input type="text" id="new-protocolo" name="protocolo" class="mascara-protocolo"
                            placeholder="#000000000" required>
                    </div>
                    <div>
                        <label for="new-telefone">Telefone:</label>
                        <input type="tel" id="new-telefone" name="telefone" class="mascara-telefone"
                ...
                <tbody>
                    @forelse($Pacientes as $p)
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge"
                                    style="background: {{ $bgCor }}; color: {{ $textoCor }}; padding: 6px 10px; border-radius: 6px; font-weight: 700; font-size: 0.70rem; letter-spacing: 0.05em; display: inline-block;">
                                    {{ $label }}
                                </span>
                            </td>
                            ...
                            <td>
                                <div style="display: flex; gap: 6px;">
                                    @if(isset($p->id))
                                        ...
                                        <button type="button" class="btn-acao-remover"
                                            onclick="if(confirm('Tem certeza que deseja remover este paciente?')) { window.location.href='{{ route('pacientes.deletar', $p->id) }}'; }">
                                            Remover
                                        </button>
                                    @endif
                                </div>
                        ...
                        @if(isset($p->id))
                            <tr id="form-editar-{{ $p->id }}" class="form-editar-expanded"
                                style="display: none; background-color: #f8fafc;">
                                <td colspan="7"
                                    style="padding: 16px 0; border-bottom: 1px solid #cbd5e1; box-shadow: inset 0 2px 4px 0 rgba(0,0,0,0.06);">
                                    <div
                                        style="background: #fff; padding: 24px; border-radius: 8px; border: 1px solid #e2e8f0; margin: 0 16px;">
                                        <h4 style="margin-top: 0; margin-bottom: 16px; color: #334155;">Editar Cadastro do Paciente
                                        </h4>
                                        <form action="{{ route('pacientes.atualizar', $p->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <div style="margin-bottom: 12px;">
                                                ...
                                                <input type="text" id="nome-{{ $p->id }}" name="nome" value="{{ $p->nome ?? '' }}"
                                                    required>
                                            </div>
                                            ...
                                            <div class="form-grid-3">
                                                <div>
                                                    ...
                                                    <input type="text" id="cpf-{{ $p->id }}" name="cpf" class="mascara-cpf"
                                                        value="{{ $p->cpf ?? '' }}" placeholder="000.000.000-00" required>
                                                </div>
                                                <div>
                                                    <label for="protocolo-{{ $p->id }}">Protocolo:</label>
                                                    <input type="text" id="protocolo-{{ $p->id }}" name="protocolo"
                                                        class="mascara-protocolo" value="{{ $p->protocolo ?? '' }}"
                                                        placeholder="#000000000" required>
                                                </div>
                                                <div>
                                                    <label for="telefone-{{ $p->id }}">Telefone:</label>
                                                    <input type="tel" id="telefone-{{ $p->id }}" name="telefone"
                                                        class="mascara-telefone" value="{{ $p->telefone ?? '' }}">
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-grid-3">
                                                <div>
                                                    ...
                                                    <input type="date" id="data_nascimento-{{ $p->id }}" name="data_nascimento"
                                                        value="{{ $p->data_nascimento ?? '' }}">
                                                </div>
                                                <div>
                                                    <label for="idade-{{ $p->id }}">Idade:</label>
                                                    <input type="number" id="idade-{{ $p->id }}" name="idade"
                                                        value="{{ $p->idade ?? '' }}">
                                                </div>
                                                <div>
                                            ...
                                            <div class="form-grid-3">
                                                <div>
                                                    ...
                                                    <input type="number" id="temperatura-{{ $p->id }}" name="temperatura"
                                                        value="{{ $p->temperatura ?? '' }}" step="0.1">
                                                </div>
                                                <div>
                                                    <label for="bpm-{{ $p->id }}">BPM:</label>
                                                    <input type="number" id="bpm-{{ $p->id }}" name="bpm"
                                                        value="{{ $p->bpm ?? '' }}" step="1">
                                                </div>
                                                <div>
                                                    <label for="oxigenacao-{{ $p->id }}">Oxigenação:</label>
                                                    <input type="number" id="oxigenacao-{{ $p->id }}" name="oxigenacao"
                                                        value="{{ $p->oxigenacao ?? '' }}" step="1">
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-grid-2">
                                                ...
                                                <div>
                                                    ...
                                                    <select id="urgencia-{{ $p->id }}" name="urgencia">
                                                        <option value="pouca" {{ $urgencia == 'pouca' || str_contains(strtoupper($urgencia), 'AZUL') ? 'selected' : '' }}>Mínima
                                                            Urgência (Azul)</option>
                                                        <option value="baixa" {{ $urgencia == 'baixa' || str_contains(strtoupper($urgencia), 'VERDE') ? 'selected' : '' }}>Pouco
                                                            Urgente (Verde)</option>
                                                        <option value="media" {{ $urgencia == 'media' || str_contains(strtoupper($urgencia), 'AMARELO') ? 'selected' : '' }}>
                                                            Urgente (Amarelo)</option>
                                                        <option value="muito_urgente" {{ $urgencia == 'muito_urgente' || str_contains(strtoupper($urgencia), 'LARANJA') ? 'selected' : '' }}>
                                                            Muito Urgente (Laranja)</option>
                                                        <option value="alta" {{ $urgencia == 'alta' || str_contains(strtoupper($urgencia), 'VERMELHO') ? 'selected' : '' }}>
                                                            Emergência (Vermelho)</option>
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-grid-2">
                                                            ...
                                                            @foreach($sintomasJson as $item)
                                                                <div class="item-sintoma-totem">
                                                                    ...
                                                                    <strong>R:</strong> <span
                                                                        style="color: {{ $item['resposta'] == 'Sim' ? '#ef4444' : '#3b82f6' }}; font-weight: bold;">{{ $item['resposta'] }}</span>
                                                                </div>
                                                            @endforeach
                                                        </div>
                                                        <input type="hidden" name="sintoma" value="{{ $sintomasRaw }}">
                                                        <span style="font-size:0.75rem; color:#64748b; font-style:italic;">* Dados
                                                            importados do Totem de Triagem (Não editável manualmente).</span>
                                                    @else
                                                ...
                                                <div>
                                                    ...
                                                    <textarea id="antecedentes_pessoais-{{ $p->id }}"
                                                        name="antecedentes_pessoais">{{ $p->antecedentes_pessoais ?? '' }}</textarea>
                                                </div>
                                            </div>

                                            <div class="form-actions">
                                                <button type="button" class="btn-cancelar"
                                                    onclick="toggleForm({{ $p->id }})">Cancelar</button>
                                                <button type="submit" style="background: #3b82f6; color: white;">Salvar
                                                    Alterações</button>
                                            </div>
                                        </form>
                                    </div>
